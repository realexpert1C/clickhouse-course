...
Создаётся одна строка, один парт(блок):
...
Теперь представим следующий блок:
...
ClickHouse не объединяет при вставке разные парты, но внутри второго парт-блока он видит только две строки, и там нет пары (-1, +1) для одинаковых Version.

Кроме этого, строки с Sign = 1 и Version = 1 в SELECT варианта А не существует — она была в одном INSERT-блоке вместе с Version 2, и ClickHouse оставил только последнюю версию.
