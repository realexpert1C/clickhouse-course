...
`SELECT * FROM tbl1;` показывает 3 строки (как в условиях ДЗ)

`SELECT * FROM tbl1 FINAL;` показывает 1 строку:

UserID	PageViews	Duration	Sign	Version
432…	6	185	1	2

FINAL “склеивает блоки” логически → применяет коллапсацию. Поэтому во всех вариантах при этих данных 
 `SELECT * FROM tbl1 FINAL;` покажет одну строку
...
---
В дополнение отмечу, что если осуществлять весь INSERT INTO одной командой для трех строк сразу, указанных в задаче данных, то нужные строки сколлапсирутся, если можно так сказать внутри одного блока "на лету".
...
🧠 Вывод: порядок INSERT напрямую влияет на поведение ___CollapsingMergeTree__
