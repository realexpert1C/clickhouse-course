...
- [Ссылки на развернутные online сервисы](#ссылки-на-развернутные-online-сервисы)
...
Логика работы DAG: 
- DAG скачивает исторические данные из MinIO и воспроизводит их в режиме реального времени (1 секунда = 1 секунда), проходя по всем секундам заданного периода
- В каждой секунде он выбирает соответствующие события и параллельно отправляет их двумя способами: в поток (Kafka) мелкими блоками и в batch (ClickHouse) через буфер с редкими крупными вставками 
- Stream имитирует near real-time ingestion 
- Batch — классический ETL с накоплением данных 
- Вставки в batch выполняются асинхронно в отдельном потоке, чтобы не тормозить цикл времени 
...
## Ссылки на развернутные online сервисы
...
- ssh доступ: `ssh demo@ssh.myclickcourse.ru`
